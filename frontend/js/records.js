...
async function loadRecords() {
    try {
        ...
        document.getElementById('labResults').textContent = records.filter(r => r.file_type === 'lab').length;
        document.getElementById('prescriptions').textContent = records.filter(r => r.file_type === 'prescription').length;
        document.getElementById('imaging').textContent = records.filter(r => r.file_type === 'imaging').length;
...
function displayRecords(records) {
    ...
    tbody.innerHTML = records.map(record => `
        <tr>
            <td><span class="badge badge-info">${getFileTypeIcon(record.file_type)}</span></td>
            ...
            <td>
                <button onclick="viewRecord(${record.id})" class="btn btn-outline" style="padding: 0.5rem 1rem; font-size: 0.85rem; margin-right: 0.5rem;">
                    👁️ View
                </button>
                <button onclick="downloadRecord(${record.id})" class="btn btn-secondary" style="padding: 0.5rem 1rem; font-size: 0.85rem;">
                    ⬇️ Download
                </button>
            </td>
        </tr>
